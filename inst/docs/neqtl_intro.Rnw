...
\SweaveOpts{prefix.string=Figs/neqtl}
...
maximum LOD score on a chromsome for LOD greater than a specified
threshold.
...
The \code{image.scanone} function is an extention of the
